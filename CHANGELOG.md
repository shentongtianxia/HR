...
#### PDF/Word简历批量导入
- 支持上传PDF和Word格式的简历文件，使用AI自动提取关键信息
- 支持同时上传多份简历文件进行批量导入
- 智能识别姓名、职位、工作经历、教育背景、技能等信息
- 在批量导入对话框中集成简历文件上传功能
- 实时显示文件解析进度和结果统计
- 显示已选择文件列表，支持多文件预览

### 后端API开发
- 创建`candidates.parseResume` API，支持PDF和Word文件解析
- 实现PDF文本提取功能（使用pdf-parse库）
- 实现Word文本提取功能（使用mammoth库）
- 集成LLM API进行简历信息结构化提取
- 创建resumeParser.ts模块，封装文件解析和AI提取逻辑

### 前端界面开发
- 在BatchImportDialog组件中添加简历文件上传区域
- 实现前端文件读取和Base64编码
- 实现文件列表展示和预览
- 实现解析进度显示和结果反馈
- 支持拖放上传多个PDF/Word文件
...
### 测试
- 编写简历解析功能单元测试（`server/resumeParser.test.ts`）
- 测试用例包括：
  - Word文档文本提取
  - AI结构化信息提取
  - 缺失信息处理
...
### 技术细节
- 安装pdf-parse@1.1.1和mammoth@1.8.0依赖库
- 使用LLM的JSON Schema响应模式，确保返回结构化数据
- 支持.pdf、.doc、.docx文件格式
- 实现错误处理和日志输出

### 部署说明
- 项目已创建检查点（版本：30245eaa）
- 所有单元测试通过
- 代码已提交到GitHub仓库
